...
The renderer (`glyphwire-host`) is built on `host_eng/`, glyphwire's own
...
work normally. It spawns `glyphwire-shell` (a pure wire client, no engine
dependency) as a separate process, which in turn launches commands with
the discovery env vars set.

```mermaid
flowchart TB
    subgraph host["glyphwire-host process (SDL3 window)"]
...
config directory at all) uses all defaults. `glyphwire-ls` has its own
...
| `host/` | `glyphwire-host` — the SDL3-windowed renderer. |
