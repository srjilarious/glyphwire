...
const OwnerIds = lsentries.OwnerIds;
const resolveAbsolutePath = lsentries.resolveAbsolutePath;
const followSymlinkKind = lsentries.followSymlinkKind;
const sortEntries = lsentries.sortEntries;
const freeEntries = lsentries.freeEntries;
const iconForEntry = lsentries.iconForEntry;
const entryMetadataJson = lsentries.entryMetadataJson;
const ownerIds = lsentries.ownerIds;
const ownerGroupText = lsentries.ownerGroupText;
const userName = lsentries.userName;
const groupName = lsentries.groupName;
...
fn classifyAndList(io: std.Io, alloc: std.mem.Allocator, operands: []const []const u8, show_hidden: bool, long_list: bool) ![]Listing {
    ...
    for (dir_ops.items) |dir_op| {
        const entries = lsentries.listDir(io, alloc, dir_op, .{ .show_hidden = show_hidden, .stat = long_list }) catch |err| blk: {
            std.log.err("glyphwire-ls: cannot open {s}: {t}", .{ dir_op, err });
            break :blk try alloc.alloc(FileEntry, 0);
        };
