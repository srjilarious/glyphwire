...
| `get_property` | request | `layer?, property` | property value | ✅ (`cursor`, `revision`, `position`, `cell_position`, `size`, `scroll`, `visibility`) |
| `set_property` | notification | `layer?, property, value` | — | ✅ (`cursor`, `position`, `cell_position`, `size`, `visibility`) — a write to a get-only property, or to `size`/`visibility` on the root layer, reports `ReadOnlyProperty` |
