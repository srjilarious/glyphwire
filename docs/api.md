...
| `get_property` | request | `layer?, property` | property value | ✅ (`cursor`, `revision`, `position`, `cell_position`, `size`, `viewport`, `scroll`, `scroll_offset`, `scrollbars`, `content_extent`, `scroll_mode`, `background`, `visibility`, `opacity`, `pty_mode`, `mouse_select`, `profile`) |
| `set_property` | notification | `layer?, property, value` | — | ✅ (`cursor`, `position`, `cell_position`, `size`, `viewport`, `scroll_offset`, `scrollbars`, `content_extent`, `scroll_mode`, `background`, `visibility`, `opacity`, `pty_mode`, `mouse_select`) — a write to a get-only property, or to `size`/`visibility`/`opacity` on the root layer, reports `ReadOnlyProperty`; `content_extent` on a `scroll_mode: "host"` layer reports `WrongScrollMode` |
...
| `mouse_select` | `{enabled}` (bool, default `false`) — whether glyphwire-host may start one of **its own** mouse text selections (drag to select, Ctrl+Shift+C to copy) on this layer. Only consulted on a context a client owns (`create_context`): the host's own root/shell context selects everywhere, as a terminal does. On a client-owned context the host stands down by default and forwards the press as a `mouse_button` event instead, because a client drawing its own panes is listening for exactly that and a selection drawn over its content would fight it — a click in a `salacommander` file pane moves that pane's cursor. A layer holding **terminal output** is the exception: `salacommander` sets this on its Ctrl+` `gw-shell --embed` panel, so a command's output there can be dragged over and copied while clicks in the file panes stay the client's. The host decides at press time, so an in-flight drag isn't affected by a later change | ✅ |
| `clip` | clip rect | 🔶 |
...
| `set_bg` | notification | `layer?, row?, col?, rows?, cols?, bg` | — | ✅ repaints only the **background** of a region — `clear`'s params and its defaulting (`row`/`col` 0, `rows`/`cols` "the rest of the layer from here"), but every cell keeps its grapheme, foreground colour, `metadata_id`, `selectable` flag, text scale and foreground icon. The primitive for **moving a highlight**: a client drawing a list with one highlighted row sends two of these (the row the cursor left, the row it landed on) instead of redrawing both rows' text and icons — salacommander's cursor keys are ~300 bytes a keystroke this way against ~3.2 KB redrawing the rows and ~54 KB redrawing the pane, which is what makes an `--ssh` session keep up (see `docs/investigations/salacommander-remote-lag.md`). `bg` is **required**: the call means "paint this background", so there is nothing sensible to default it to, and a missing one is a parse error. A cell's background is one slot, so a cell whose background was an image or an icon (`draw_image`/`draw_icon` without `foreground`) takes the colour like any other — this overwrites rather than merging, the way `write_text` already does with `grapheme`/`style`. A *foreground* icon (`draw_icon` with `foreground: true`) is a separate field and survives, which is what lets a listing row keep its icon. Alpha decides transparency as everywhere else, so an `a` of 0 puts a region back to transparent without blanking it. Batchable, and meant to be: the two rows a highlight moves between go out in one frame |
...
sorted. This works on **any layer of the visible context**, not just its
root: the pointer is resolved against the top-most visible layer whose
bounds contain it (matching how the renderer composites them) and only
that layer's tables are tested, so a `gw-ls -l` table lands a header
click whether it was drawn on a plain shell session's root layer, in a
`gmux` pane, or in `salacommander`'s embedded `gw-shell --embed` panel.
...
- **Disallowed sub-methods:** `batch` (no nesting) and
  ...
  *is* allowed: it carries no payload, so a client can clear a region and
  ...
  / metadata commands.
- **A notification-form batch costs one frame and no round trip**, so
  over a remote session (`--ssh`) the budget to watch is *payload*, not
  message count — a full-screen repaint is one frame of tens of
  kilobytes of JSON, which is free on a local socket and is not through
  ...
  See `docs/investigations/salacommander-remote-lag.md` for a worked
  example.
