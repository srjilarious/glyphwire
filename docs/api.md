...
| `table_set_sort` | notification | `layer?, table, column?, direction?` | — | ✅ `column: null` or `direction: "none"` (the default) both mean "back to insertion order"; otherwise `"ascending"`/`"descending"` on that column's `SortKey`. Repaints **in place** (`core.Table.repaint`) — the row set is unchanged, so it redraws the table at its current position without scrolling the layer a second time; a table taller than the viewport re-sorts the part that's on screen, rows already in scrollback keep their prior order. The active sort column's header draws a direction arrow after its name (`"Name ▴"` / `"Name ▾"`) and widens by two cells so the arrow never clips the name; a merely `sortable` column that isn't the current sort shows nothing extra. This is also the mutation glyphwire-host runs itself on a header click (see Table interactivity below) |
...
table's pinned `top_live` (which `Layer.scrollOne` keeps accurate as
output pushes the table up) plus the layer's scrollback offset, so it
works wherever the header is actually on screen — including after output
has scrolled the table or the user has scrolled the view back to reach
the header. Scope of the first cut: tables on the **visible context's
