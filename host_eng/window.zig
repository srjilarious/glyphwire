...
pub const WindowState = struct {
    ...
    /// Re-queries window size and pixel size from SDL and recomputes
    /// `scale_factor`. `SDL_GetWindowDisplayScale` is deliberately not
    /// consulted: under Wayland fractional scaling it disagrees with the
    /// real framebuffer/window ratio, and that ratio is what every
    /// coordinate conversion here actually needs.
    ...
    pub fn framebufferRect(self: *const WindowState) core.RectI {
